...
// Atlas Dev Pipeline database ID: ce6fbf1bee30433da9e6b338552de7c9
// Data source ID: 1460539c-7002-447a-a8b7-17bba06c6559
const NOTION_PIPELINE_DB = process.env.NOTION_PIPELINE_DB || '1460539c-7002-447a-a8b7-17bba06c6559';
...
async function syncToNotion(discussion: Discussion): Promise<string | null> {
  if (!NOTION_API_KEY || !NOTION_PIPELINE_DB) {
    return null;
  }
  ...
  try {
    ...
    // In production, store notion_page_id in discussion
    if (discussion.notion_url) {
      ...
      const response = await safeNotionCall(() => notion.pages.create({
        ...
        properties: {
          ...
          'Status': { select: { name: formatStatus(discussion.status) } },
          'Type': { select: { name: capitalize(discussion.type) } },
          'Priority': { select: { name: discussion.priority } },
          'Requestor': { select: { name: discussion.requestor } },
          'Handler': { select: { name: discussion.assignee } },
          'Thread': { rich_text: [{ text: { content: threadMd.substring(0, 2000) } }] },
          'Work Queue': discussion.wq_url ? { url: discussion.wq_url } : { url: null },
          'Dispatched': { date: { start: discussion.created } },
        },
      }));
  ...
  switch (name) {
    case 'dispatch_work': {
      ...
      const discussion: Discussion = {
        ...
        status: 'dispatched',
        requestor: 'atlas', // Default, can be overridden
        assignee: 'pit-crew',
...
async function main() {
  ...
  console.error('Pit Crew MCP server running');
}
